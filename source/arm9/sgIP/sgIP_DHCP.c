...
void sgIP_DHCP_BeginDgram(int dgramtype)
{
    ...
    // add some necessary options... - by default add the dhcp message type, host name, class id,
    ...
    if (dhcp_requestDNS)
        dhcp_p->options[dhcp_optionptr++] = 6; // dns server
...
void sgIP_DHCP_Release(void)
{
    // call to dump our DHCP address and leave.
...
int sgIP_DHCP_Update(void)
{
    ...
    if (p)
    {
        while (1)
        {
            ...
            if (l == -1)
                break;
            if (p->op != 2 || p->htype != 1 || p->hlen != 6 || p->xid != dhcp_tid)
                continue;
            // check magic cookie
            if (p->options[0] != 0x63 || p->options[1] != 0x82 || p->options[2] != 0x53
                || p->options[3] != 0x63)
                continue;
            ...
            while (i < l)
            {
                ...
                switch (n)
                {
                    ...
                    case 53: // message type, variable length, 2+n
                        ...
                        if (dhcp_state == 0)
                        {
                            if (p->options[i] != DHCP_TYPE_OFFER)
                                l = -1;
                        }
                        else
                        {
                            if (p->options[i] == DHCP_TYPE_ACK)
                            {
                                ...
                                SGIP_DEBUG_MESSAGE(("IP%08X SM%08X GW%08X", dhcp_rcvd_ip,
                                                    dhcp_rcvd_snmask, dhcp_rcvd_gateway));
                                if (dhcp_requestDNS)
                                {
                                    dhcp_int->dns[0] = dhcp_rcvd_dns[0];
                                    dhcp_int->dns[1] = dhcp_rcvd_dns[1];
                                    dhcp_int->dns[2] = dhcp_rcvd_dns[2];
                                    SGIP_DEBUG_MESSAGE(("DNS %08X %08X %08X", dhcp_rcvd_dns[0],
                                                        dhcp_rcvd_dns[1], dhcp_rcvd_dns[2]));
                                }
                    ...
                    case 6: // dns servers, variable length 2+n
                        if (dhcp_requestDNS && !dhcp_state)
                        {
                            ...
                            {
                                dhcp_rcvd_dns[n] =
                                    (dhcp_rcvd_dns[n] >> 8) | (p->options[i++] << 24);
                                dhcp_rcvd_dns[n] =
                                    (dhcp_rcvd_dns[n] >> 8) | (p->options[i++] << 24);
                                dhcp_rcvd_dns[n] =
                                    (dhcp_rcvd_dns[n] >> 8) | (p->options[i++] << 24);
        ...
        if (send || (sgIP_timems - dhcp_timelastaction) > SGIP_DHCP_RESENDTIMEOUT)
        {
            if (dhcp_state == 0)
                ...
                sgIP_DHCP_BeginDgram(DHCP_TYPE_REQUEST);
            sgIP_DHCP_SendDgram();
        }
    }
    else
...
void sgIP_DHCP_Terminate(void)
{
    // kill the process where it stands; deallocate all DHCP resources.
    if (dhcp_socket)
