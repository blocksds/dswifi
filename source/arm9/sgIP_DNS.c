...
void sgIP_DNS_Timer1000ms(void)
{
    ...
    for (int i = 0; i < SGIP_DNS_MAXRECORDSCACHE; i++)
    {
        if (dnsrecords[i] != NULL
            && ((dnsrecords[i]->flags & (SGIP_DNS_FLAG_RESOLVED | SGIP_DNS_FLAG_PERMANENT))
                == SGIP_DNS_FLAG_RESOLVED))
        {
...
int sgIP_DNS_isipaddress(const char *name, unsigned long *ipdest)
{
    ...
    switch (ndot)
    {
        case 0:
            ...
            break;
        case 1:
            if (g[0] >= 0x100 || g[1] >= 0x1000000)
                return 0;
            out_addr = (g[0] << 24) | g[1];
            break;
        case 2:
            if (g[0] >= 0x100 || g[1] >= 0x100 || g[2] >= 0x10000)
                return 0;
            out_addr = (g[0] << 24) | (g[1] << 16) | g[2];
            break;
        case 3:
            if (g[0] >= 0x100 || g[1] >= 0x100 || g[2] >= 0x100 || g[3] >= 0x100)
                return 0;
...
{
    SGIP_INTR_PROTECT();
    for (int rec = 0; rec < SGIP_DNS_MAXRECORDSCACHE; rec++)
    {
        if (dnsrecords[rec] != NULL
            && (dnsrecords[rec]->flags & (SGIP_DNS_FLAG_ACTIVE | SGIP_DNS_FLAG_RESOLVED))
                   == (SGIP_DNS_FLAG_ACTIVE | SGIP_DNS_FLAG_RESOLVED))
        {
            for (int ali = 0; ali < dnsrecords[rec]->numalias; ali++)
            {
...
static void sgIP_ntoa(unsigned long ipaddr)
{
    int i, n;
...
static sgIP_DNS_Hostent *sgIP_DNS_GenerateHostentIP(unsigned long ipaddr)
{
    sgIP_ntoa(ipaddr);
...
static int sgIP_DNS_genquery(const char *name)
{
    int i, j, c, l;
    ...
    while (1)
    {
        ...
        if (name[i] == 0 || i >= 255)
            break;
        if (c == 0)
            return 0; // this should never happen (unless there's really invalid input with 2 dots
                      // next to each other.)
        i++;
    }
...
dns_listenonly:
            ...
            if (i == -1)
            {
                // no reply, retry
                retries++;
                if (retries >= SGIP_DNS_MAXRETRY)
                {
                    // maybe try another server? for now just quit.
            ...
            if (i < 12 || sain.sin_addr.s_addr != serverip || sain.sin_port != htons(53))
            {
                // suspicious.

                // yay! a goto! - go back and see if we can get a more official response.
                goto dns_listenonly;
            }
            ...
            // parse response.
            {
                ...
                while (a)
                {
                    if (nalias < SGIP_DNS_MAXALIASES)
                        sgIP_DNS_CopyAliasAt(rec->aliases[nalias++], resdata_c - responsedata);
                    do
                    {
                        j = resdata_c[0];
                        if (j > 63)
                    ...
                    int ttl = (resdata_c[4] << 24) | (resdata_c[5] << 16) | (resdata_c[6] << 8)
                              | resdata_c[7];
                    if (ttl < 0)
                        ttl = 0;
